...
const FileIcon = ({ type, name, isHidden, onClick }) => (
  <div
    onClick={onClick}
    className={`flex flex-col items-center gap-1 group cursor-pointer transition-all hover:scale-105 active:scale-95 touch-manipulation min-h-[44px] min-w-[60px] p-2 ${isHidden ? 'opacity-40' : ''}`}
  >
    {type === 'dir' ? (
      <Folder
        size={28}
